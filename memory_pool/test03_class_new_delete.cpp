...
int main() {
    ...
    for (int i = 0; i < LOOPS; ++i) {
        for (auto &ptr: ptr_array1) { ptr = new normal::Obj; }
        for (auto ptr: ptr_array1) { delete ptr; }
    }
    t2 = rdtsc();
    ...
    t1 = rdtsc();
    for (int i = 0; i < LOOPS; ++i) {
        for (auto &ptr: ptr_array2) { ptr = new pooled::Obj; }
        for (auto ptr: ptr_array2) { delete ptr; }
    }
    t2 = rdtsc();
    cout << setw(3) << (t2 - t1) / LOOPS / LEN
